...
{  
  ...
  do {
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    xsem_wait(a->sem_cindex,__LINE__,__FILE__);
    n = a->buffer[*(a->pcindex) % Buf_size];
    *(a->pcindex) += 1;
    xsem_post(a->sem_cindex,__LINE__,__FILE__);
...
int main(int argc, char *argv[])
{
  ...
  pthread_t t[p];
  dati a[p];
  sem_t sem_free_slots, sem_data_items, sem_cindex;
  
  // inizializzazione semafori
  xsem_init(&sem_free_slots, 0, Buf_size,__LINE__,__FILE__);
  xsem_init(&sem_data_items, 0, 0,__LINE__,__FILE__);
  xsem_init(&sem_cindex,0,1,__LINE__,__FILE__);
