...
int main(int argc, char *argv[]) {
  ...
  for(int i=0;i<p;i++) {
    ...
    if(pid==0) {// figlio
      close(up[0]);  //chiudo perchè non serve la lettura(qua il figlio ci scrive)
      close(down[1]); //chiudo perchè non serve la scrittura(qua il figlio legge)
      int tot=0;
      while(true) {
        int x;
  ...
  close(up[1]); //chiudo perchè non serve la scrittura(qua il padre ci legge)
  close(down[0]); //chiudo perchè non serve la lettura(qua il padre ci scrive)
  
  // i padre inizia a leggere gli interi dal file
  FILE *f = xfopen(argv[1],"rt",__FILE__,__LINE__);
  while(true) {
    int x;
    int e = fscanf(f,"%d",&x); //leggo un intero da file e lo salvo
    if(e==0|| e==EOF) break;
    ssize_t ex = write(down[1],&x,sizeof(int)); //scrive l'intero letto 
    if(ex!=sizeof(int)) die("Errore scrittura pipe (padre)");
  }
  fclose(f); //chiudo il file
  close(down[1]); //chiudo pipe di scrittura
